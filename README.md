...
- Migrates settings from earlier Searcher versions without losing a selected search engine.

## Install

### Community Plugins

Install **Web Searcher** from Obsidian's Community Plugins browser after the plugin is published.

### Manual installation

1. Create `.obsidian/plugins/web-searcher/` in your vault.
2. Copy `manifest.json`, `main.js`, and `styles.css` into that directory.
3. Enable **Web Searcher** under **Settings > Community plugins**.
...
Choose a search engine in **Settings > Community plugins > Web Searcher**. For a custom engine, enter an HTTP(S) URL such as:

```text
https://www.google.com/search?q={query}
```

`{query}` is replaced with the encoded selected text. If it is omitted, Web Searcher appends the encoded text to the URL, which supports existing URLs such as `https://www.google.com/search?q=`.
...
- 可自动迁移早期 Searcher 版本的搜索引擎设置。

## 安装

### 社区插件

插件发布后，可在 Obsidian 的社区插件浏览器中搜索并安装 **Web Searcher**。

### 手动安装

1. 在 Vault 中创建 `.obsidian/plugins/web-searcher/`。
2. 将 `manifest.json`、`main.js` 和 `styles.css` 复制到该目录。
3. 在 **设置 > 第三方插件** 中启用 **Web Searcher**。
...
在 **设置 > 第三方插件 > Web Searcher** 中选择搜索引擎。使用自定义引擎时，请填写 HTTP(S) URL，例如：

```text
https://www.google.com/search?q={query}
```

`{query}` 会被替换为经过 URL 编码的选中文字。若未使用该占位符，Web Searcher 会把编码后的文字追加到 URL 末尾，因此也兼容 `https://www.google.com/search?q=` 这类地址。
